...
module.exports = function (options) {
  ...
  var config = {
    baseGit: 'https://github.com/ericfu88/webhook-generate',
...
function downloadRepo(config, tmpFolder, callback) {
  ...
  var repoRequest = request(config.baseGit + '/archive/master.zip');
...
function generateProject(config, callback) {
  ...
  wrench.copyDirSyncRecursive('.wh-generate/webhook-generate-master', process.cwd() + '/' + dirName, {
      forceDelete: true
  });
